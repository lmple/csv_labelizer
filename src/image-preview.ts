...
import { CsvMetadata, RowData } from './types';

let currentCsvDir: string = '';
let imageColumnIndex: number | null = null;

export function initializeImagePreview(metadata: CsvMetadata) {
    currentCsvDir = metadata.csv_dir;
    imageColumnIndex = metadata.image_column;
}

export async function loadImage(rowData: RowData) {
