...
// import { getCurrentWindow } from '@tauri-apps/api/window'; // TODO: Re-enable for close warning
...
async function saveCurrentRow() {
    ...
    while (retryCount <= maxRetries) {
        try {
            ...
            let errorType: 'permission' | 'disk_space' | 'file_locked' | 'file_modified' | 'unknown' = 'unknown';
 ...
 */
async function saveAsCurrentRow(fields: string[], index: number): Promise<boolean> {
    try {
        // Show save dialog
        ...
        const metadata: CsvMetadata = await invoke('open_csv', { path: newPath });
        const classValues: ClassValuesMap = await invoke('get_class_columns');
